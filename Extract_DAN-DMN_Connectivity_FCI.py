...
for subject in subjects:
    ...
    for session in sessions:
        ...
        for condition in conditions:
            print(condition)
            try:
                graph = pd.read_csv(f'{deriv_dir}/idconn-0+unknown_reas/sub-{subject}/ses-{session}/func/sub-{subject}_ses-{session}_task-fci_condition-{condition}_desc-Schaefer400_2mm_corrmat.tsv', 
                                    sep='\t', 
                                    header=0, 
                                    index_col=0)
                ...
                dan_def_node_conn.to_csv(f'{deriv_dir}/idconn-v0.1-presub+90.g6973d15/fci_dan-def_node-connectivity.tsv', sep='\t')
