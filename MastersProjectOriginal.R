...
#Adding 5 profiles produces errors, 4 is the limit
...
#Filtering Individuals w/ a cprob > 0.7
sum(tab[,1]>=0.7) #n = 5
sum(tab[,2]>=0.7) #n = 25; 1 miss
sum(tab[,3]>=0.7) #n = 74
sum(tab[,4]>=0.7) #n = 74
...
#Group 1 has 6 individuals, G2 (High Spatial, Low Math) has 17, G3(High Math, Average Spatial) has 26, G4 (Low Math & Spatial) has 58
project[c(2,4:5,7)]  = project[c(2,4:5,7)] %>% sapply(function(x) as.factor(x))
#Determining which individuals/rows are in which group
...
check = data.frame(t1$Years, ndata$Years)
miss = which(is.na(t1[12]), arr.ind = T)
miss  = data.frame(miss)
miss = unique(miss$row)
miss = t1[miss, 24]
#9 Low Anxiety & 4 High Math missing
table(miss)
table(t1[24])
...
#cor(within$withindefans,t1$Years, use = "pairwise.complete.obs")
library(lavaan)
mod = '
...
#109-136=28; 313-335; 23 = 51; 2601 -51 = 2550
...
danb = danb[,combined]
#First row corresponds to region labels. This code selects all labels related ot dan.
...
#47 van regions = 2209 total, subtract 1 to 1 correlations = 2162
#85-108= 24; 285-312 = 28 = 2704 - 52 = 2652
van = node[(node$X=='1'),]
...
#1-41 = 41; 201-236 = 36; 77 = 5929 - 77 = 5852
...
a = lavaan(mod, data = within)
summary(model, fit.measures = T)




summary(lm(withindefans~Anxiety.Profiles+ Sex + Years + Income + Age + Ethnicity  + GPA + BK
           , data = within))
...
dandefnans~Anxiety.Profiles + Sex + Years + Income + Age + Ethnicity  + GPA + BK
dandefans~~dandefans
dandefans~1
'
